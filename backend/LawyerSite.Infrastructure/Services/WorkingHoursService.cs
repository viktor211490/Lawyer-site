...
public class WorkingHoursService : IWorkingHoursService
{
    ...
    public async Task<List<TimeSlotDto>> GetAvailableSlotsForDayAsync(DateTime date)
    {
        ...
        while (currentMinutes + schedule.SlotDurationMinutes <= endMinutes)
        {
            ...
            var slotLocalMoscow = MoscowTimeProvider.ConvertToMoscow(slotDateTime);
            var moscowNow = MoscowTimeProvider.GetMoscowNow();
            ...
            slots.Add(new TimeSlotDto(
                // Всегда отдаём время в московской зоне, независимо от TZ сервера
                MoscowTimeProvider.ConvertToMoscow(slotDateTime),
                !isTaken && !isBlocked,
