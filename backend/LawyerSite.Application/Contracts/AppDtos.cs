namespace LawyerSite.Application.Contracts;
...
)
{
    ...
    {
        // В доменной модели AppointmentTime трактуем как московское локальное время (Kind = Unspecified).
        // Конвертируем его в UTC, чтобы корректно хранить в БД и исключить "скачки" на клиентах.
        ...
        {
            // Moscow time zone id differs across OS. Try Windows, then IANA.
            TimeZoneInfo? tz = null;
            try { tz = TimeZoneInfo.FindSystemTimeZoneById("Russian Standard Time"); } catch { /* ignore */ }
            try { tz ??= TimeZoneInfo.FindSystemTimeZoneById("Europe/Moscow"); } catch { /* ignore */ }
            tz ??= TimeZoneInfo.Utc;
            return TimeZoneInfo.ConvertTimeToUtc(AppointmentTime, tz);
        }

        // Local -> UTC using server local zone
