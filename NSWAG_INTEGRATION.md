# NSwag Интеграция - LawyerSite Проект
...
LawyerSite/
├── src/
│   ├── LawyerSite.Api/           # .NET 8 Web API backend
│   │   ├── Controllers/          # API контроллеры
│   │   ├── Program.cs            # Точка входа
│   │   ├── nswag.json            # Конфигурация генерации клиента
│   │   └── LawyerSite.Api.csproj # NSwag пакеты и таргеты
│   │
│   └── LawyerSite.Web/           # Angular 19 frontend
│       └── src/
│           └── app/
│               ├── services/
│               │   ├── client/
...
**Файл:** `src/LawyerSite.Api/LawyerSite.Api.csproj`
...
```xml
<PropertyGroup>
  <SpaRoot>..\LawyerSite.Web</SpaRoot>
...
**Файл:** `src/LawyerSite.Api/nswag.json`
...
```json
{
  ...
  "codeGenerators": {
    "openApiToTypeScriptClient": {
      ...
      "typeStyle": "Interface",
      "enumStyle": "Enum",
      "output": "../LawyerSite.Web/src/app/services/client/web-api-client.ts"
    }
  }
}
```
...
**Файл:** `src/LawyerSite.Web/package.json`

```json
{
  "scripts": {
    "start": "ng serve --proxy-config src/proxy.conf.js",
    "generate-client": "cd ../LawyerSite.Api && nswag run nswag.json"
  }
}
```

**Файл:** `src/LawyerSite.Web/src/proxy.conf.js`
...
**Файл:** `src/LawyerSite.Web/src/app/services/client/web-api-client.ts`
...
**Файл:** `src/LawyerSite.Api/Program.cs`
...
```bash
# Из корня проекта
dotnet run --project src/LawyerSite.Api
```
